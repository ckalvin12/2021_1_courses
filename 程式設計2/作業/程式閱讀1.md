...
public class ForLoop {
	...
	public static void main(String[] args) {
		System.out.println("長夜漫漫，無心睡眠，請給他設定一個睡醒的年限");
		Scanner scan = new Scanner(System.in); 
...
public class BinaryChop {
	...
	public static void main(String[] args) {
		...
		System.out.println("最後生成的亂數="+item);

		
		int aim_item = item; 
		System.out.println("準備查找的目標數位="+aim_item);
		int start = 0; 
...
public class Jitutonglong {
	...
	public static void main(String[] arg) {
		...
		System.out.println("雞的數量為" + chick + "，兔子的數量為" + rabbit);
	}
}
...
public class SunziDingli {
	...
	public static void main(String[] arg) {
		...
		for (int number : numbers) { 
			System.out.println("符合孫子定理的整數number=" + number);
		}
	}
...
public class BigNewton {
	...
	private static void testSqrtByDouble() {
		...
		System.out.println("雙精度數開方運算，原始數位=" + number + ", 它的平方根=" + root);
	}
	...
	private static void testSqrtByBigDecimal() {
		...
		System.out.println("大小數開方運算，原始數位=" + number + ", 它的平方根=" + root);
	}
...
public class ExactPai {
	...
	private static void calculateByDouble() {
		...
		System.out.println("割圓次數=" + n + ", 內接正N邊形的邊數=" + edgeNumber);
		
		double π_rough = getPaiRough(n);
		System.out.println("粗略的圓周率數值=" + π_rough);
	}
	...
	private static void calculateByBigDecimal() {
		...
		System.out.println("精確的圓周率數值=" + π_exact);
	}
...
public class RecursionFactorial {
	...
	private static void calculateFactorialByLong() {
		...
		System.out.println(n+"!的長整數階乘結果="+resultLong);
	}
	...
	private static void calculateFactorialByBigInteger() {
		...
		System.out.println(n+"!的大整數階乘結果="+resultBig);
	}
...
public class StrFormat {
	...
	public static void main(String[] arg) {
		...
		String fromChar = String.format("格式化字元的字串：%c", 'A');
		...
		String fromOct = String.format("格式化十六進位數的字串：%o", 255);
		System.out.println("fromOct=" + fromOct);
		
		String fromHex = String.format("格式化八進位數的字串：%x", 255);
		...
		String manyVariable = String.format(
				"以下字串包括了多個變數值：%s，%c，%b，%d，%o，%x，%f", "Hello", 'A', false, 255,
				255, 255, 3.14);
		...
		String fromLenth = String.format("格式化固定長度（默認右對齊）的整數字串：(%8d)", 255);
		System.out.println("fromLenth=" + fromLenth);
	
		String fromLeft = String.format("格式化固定長度且左對齊的整數字串：(%-8d)", 255);
		System.out.println("fromLeft=" + fromLeft);
	
		String fromZero = String.format("格式化固定長度且左補0的整數字串：(%08d)", 255);
		System.out.println("fromZero=" + fromZero);

		
		String fromRepeat1 = String.format("重要的事情說三遍：%s，%s，%s", "別遲到", "別遲到",
				"別遲到");
...
public class IcnoExtract {
	...
	public static void main(String[] arg) {
		...
		for (String icno : icnoArray) {
			...
			System.out.println(String.format("%s的校驗結果為：%s", icno, result));
		}
	}
	...
	private static String extractIc(String icno) {
		...
		String[] areaNames = { "東城區", "西城區", "朝陽區", "豐台區", 
				"石景山區", "海澱區", "門頭溝區", "房山區", "通州區", "順義區", 
				"昌平區", "大興區", "懷柔區", "平谷區", "密雲區", "延慶區" };
		...
		return String.format("居住地址是%s，出生日期是%s，出生序號是%s，性別是%s。",
...
public class KmpSearch {
	...
	private static int[] findStringByBF(String src, String dest) {
		...
		System.out.println("BF演算法的總比較次數 = " + total_count);
		return posArray;
	}
	...
	private static int[] findStringByKMP(String src, String dest) {
		...
		System.out.println();
		System.out.println("KMP演算法的關鍵字比較次數 = " + total_count);
		...
		System.out.println("KMP演算法的總比較次數 = " + total_count);
		return posArray;
	}
}
...
public class ParseAddress {
	public static void main(String[] arg) {
		String[] infoArray = new String[]{
				"張三 11900000000 北京市海澱區雙清路30號",
				"059100000000,福建省福州市閩侯縣上街鎮工貿路3號,李四",
				"11900000000 王五 四川省涼山彝族自治州西昌市大水井12號",
				"西藏自治區阿裡地區噶爾縣獅泉河鎮迎賓大道26號,趙六,059100000000",
				"劉七 內蒙古自治區阿拉善盟額濟納旗達來呼布鎮黑水城遺址 11900000000"
		};
	...
	private static void parseReceiverInfo(String info) {
		...
		System.out.println(String.format("姓名：%s, 電話：%s, 收件地址：%s", name, phone, address));

		
		String[] areaArray = new String[]{"", address};
		
		areaArray = getAreaName(areaArray[1], new String[]{"省", "自治區"});
		String province = areaArray[0]; 
		
		areaArray = getAreaName(areaArray[1], new String[]{"自治州", "地區", "盟", "市"});
		String city = areaArray[0]; 
		
		areaArray = getAreaName(areaArray[1], new String[]{"縣", "市", "區", "旗"});
		String district = areaArray[0]; 
		String detail = areaArray[1]; 
		if (province.length() <= 0) { 
			province = city;
		}
		System.out.println(String.format("省份：%s, 地市：%s, 區縣：%s, 詳細地址：%s", 
